...
## Infrastructure
- **Frontend Hosting**: Namecheap Stellar shared hosting plan
  - Manual uploads to shared host from `/frontend/HTML/` folder
  - Static files served from public_html
- **Backend Hosting**: VPS with Docker containers
  - FastAPI backend container
  - PostgreSQL database container  
  - ~~Mailcow server for 4 mail domains~~ → **Migrating to shared hosting email with Roundcube**
  - All behind nginx with 2 sites-available configs
- **DNS**: GoDaddy for domain management
- **Email**: ✅ Roundcube installed on shared hosting, email accounts working for sending. Need DNS MX records update at GoDaddy to point to Namecheap mail servers for receiving
...
- Core Skills/Technical Skills (matched to job requirements)
...
- **Storage**: Cloudflare R2 for document storage (cheap, S3-compatible)

## Implementation Status

### ✅ Completed Features (Session 3)
- **Professional UI**: Modern interface with User Chat Area, resume table, and save dialogs
- **Cloudflare R2 Storage**: Complete file storage system with boto3 integration
- **Authentication**: Signup, login, email verification with bcrypt password hashing
- **Password Reset**: Complete forgot password flow with email tokens and reset page
- **Resume Management**: Upload (PDF/TXT/DOCX), download, delete, default assignment
- **AI Resume Generation**: Real Gemini AI integration with comprehensive resume context
- **Document Downloads**: PDF and Word generation with proper formatting and styling
- **Profile Links**: Display and integration into resume generation workflow
- **Progress Tracking**: Visual progress bars for resume generation process
- **Profile Scraping**: Microservice for LinkedIn/GitHub/website data collection
- **Support Integration**: "Need Help?" email link in main navigation
- **Database Schema**: Complete models for R2 storage, generated resumes, and user profiles

### 🚧 Session 4 Progress (2025-07-29) - CURRENT SESSION
- **✅ Password Reset URL Fix**: Fixed API subdomain issue in password reset emails
- **✅ Database Initialization**: Enhanced init_db.py to auto-create database if missing and handle pgcrypto extension gracefully
- **✅ Scraper Dependencies**: Added psycopg2-binary to scraper requirements.txt
- **✅ UI Toggle Feature**: Added toggle between Profile Links and User Context sections with cyan highlighting
- **✅ Resume Formatting Fix**: Fixed company name display issues (asterisks showing instead of bold headers)
- **✅ Sample Data Cleanup**: Removed hardcoded wireframe data, replaced with real API integration
- **✅ Set Default Resume Bug Fix**: Fixed SQLAlchemy async/await compatibility issue causing 500 errors
- **✅ Lambda Function Code Fix**: Fixed duplicated code in AWS Lambda chatbot function causing syntax errors
- **✅ SECURITY INCIDENT RESPONSE**: Detected and stopped live SQL injection attack from 78.153.140.66 attempting to compromise PostgreSQL database
- **✅ Infrastructure Security**: Removed exposed PostgreSQL port, conducted full system backdoor scan, confirmed attack contained to database container

#### Files Modified in Session 4:
- `/backend/app/routes/auth.py` - Fixed password reset URL generation
- `/backend/init_db.py` - Added database auto-creation and graceful extension handling  
- `/scraper/requirements.txt` - Added psycopg2-binary dependency
- `/External/HTML/app.html` - Added toggle UI, fixed formatting, removed sample data
- `/backend/app/routes/resumes.py` - Fixed set-default endpoint SQLAlchemy async compatibility
- `/lambda/chatbot/lambda_function.py` - Fixed duplicated code causing syntax errors

#### Backend Status:
- ✅ Database creation working automatically
- ✅ Backend container running successfully  
- ✅ Password reset functionality fixed
- ✅ Set default resume functionality working correctly
- ✅ AWS Lambda function tests passing (code duplication fixed)
- ⚠️ Scraper needs container rebuild for psycopg2-binary

### 🚧 Known Issues
- **AWS Lambda 404 Error**: Chatbot frontend getting 404 when calling Lambda URL (https://ojgx74ohzyz47etmxf6cfq6t3u0mwkhn.lambda-url.eu-north-1.on.aws/) - needs CloudWatch logs investigation
- **Profile Scraping**: Needs testing with existing user accounts
- **Scraper Container**: Needs rebuild to include psycopg2-binary dependency
- **VPS Memory Issues**: Mailcow email server causing OOM kills of PostgreSQL container - migrating email to shared hosting with Roundcube

### 🔧 Technical Details - Set Default Resume Fix
**Problem**: SQLAlchemy 2.0 async/await compatibility issue causing `greenlet_spawn has not been called` errors
**Root Cause**: Using `.scalar_one_or_none()` and `.scalars().all()` methods in async contexts
**Solution**: 
- Replaced `result.scalar_one_or_none()` with `result.first()` for single record queries
- Replaced ORM object loading/modification pattern with direct SQL UPDATE statements
- Used simpler SQL operations to avoid async/sync conflicts
**Files Changed**: `/backend/app/routes/resumes.py` - set_default_resume() function
**Result**: "Set Default" button now works without errors, frontend updates immediately

### 📋 Next Steps - Recruiter Platform
- [ ] Design interactive map interface for talent discovery
- [ ] Implement tiered subscription model ($99-$699/month)
- [ ] Add relocation willingness and remote work preferences to user profiles
- [ ] Build candidate search and filtering dashboard
- [ ] Integrate payment processing and subscription management
- [ ] Add recruitment-focused profile indicators (visa status, availability, etc.)

## Personal Resume Chatbot Architecture (Session 4 - 2025-07-27)

### Hybrid Architecture Decision
- **VPS (Current Setup)**: Main app, auth, user profiles, chatbot link generation, PostgreSQL database
- **AWS Lambda (New Chatbot Service)**: AI conversation engine, scalable per-conversation pricing, fast global response

### Chatbot Implementation Details

#### Current Features (✅ Completed):
- **Chatbot Route**: `/api/chatbot/{token}` serves personalized chat pages by user's chatbot token
- **Clean UI**: Dark theme with cyan accents matching main app design
- **Personalized Title**: "John Smith's Personal Resume Chat - What would you like to know?"
- **Simple Interface**: Chat bubbles, text input, close button, typing indicators
- **Mobile Responsive**: Works across all devices
- **Database Integration**: Finds users by chatbot_link token from existing User model

#### Flow Architecture:
```
1. User visits chatbot link (e.g., myresume.chat/api/chatbot/abc123)
2. VPS serves personalized chatbot page with user's name
3. Chat messages hit AWS Lambda API for AI responses  
4. Lambda queries VPS for user context (resumes, skills, scraped data)
5. AI generates personalized responses based on user's background
```

#### Benefits of AWS Lambda for Chatbot:
- **Cost Efficient**: Pay per chatbot interaction vs. 24/7 server costs
- **Auto-Scaling**: Handle viral chatbot sharing without server crashes  
- **Performance**: Lambda cold starts acceptable for chat (1-2 seconds)
- **Separation**: Chatbot traffic doesn't impact main app performance
- **Global Reach**: CloudFront edges serve chat responses worldwide

#### Next Implementation Steps:
- [ ] Create AWS Lambda function for AI chat responses
- [ ] Integrate user resume and scraped data into chat context
- [ ] Connect Lambda to VPS database for user data retrieval
- [ ] Implement AI model integration (Gemini) in Lambda function
- [ ] Add conversation context management for multi-turn chats

### Future Document Generation Platform Vision
**Concept**: Parameterized document generation system with multiple AI models
- **Document Templates Table**: Rules, formatting, AI prompts for different document types
- **AI Models Table**: Multiple providers (Gemini, GPT-4, Claude) with cost tracking
- **Use Cases**: Resumes, cover letters, business proposals, legal documents, creative writing
- **Revenue Model**: Freemium, API access, white label, enterprise custom templates

### 🔧 Technical Improvements
- [ ] Add email configuration for production SMTP
- [ ] Implement comprehensive error handling for all services
- [ ] Add file format validation on upload
- [ ] Optimize mobile responsive design for recruiter dashboard

## OpenAI Migration Plan (Session 4 - 2025-07-30)

### 🎯 Next Task: Migrate Lambda Chatbot from Gemini to OpenAI GPT-4.1 Nano
**Status**: Ready to implement - user signing up for OpenAI API account
...
1. **Environment Variable**: Change `GEMINI_API_KEY` to `OPENAI_API_KEY` in Lambda
2. **API Endpoint**: Replace Gemini REST API with OpenAI's chat completions endpoint
3. **Request Format**: Update payload structure for OpenAI's API format
4. **Model**: Use `gpt-4.1-nano` model identifier
5. **Response Parsing**: Adjust response extraction from OpenAI's format

#### Files to Modify:
- `/lambda/chatbot/lambda_function.py` - Replace `generate_ai_response()` function
- AWS Lambda environment variables configuration

#### Current Token Usage Estimate:
- Input: ~1,200-1,500 tokens (system prompt + user context + question)
- Output: ~100-300 tokens (concise responses)
- Total cost per interaction: Under $0.0003

## Recruiter Platform Architecture (Session 5 - 2025-08-04)

### 🎯 Complete Recruiter Platform Design
**Status**: Database schema designed, wireframe in progress

### Revenue Model & Global Strategy
- **Subscription Tiers**: $99/$299/$699 monthly plans
- **Payment Processing**: Stripe for global reach (39+ countries, 135+ currencies)
- **Cost Structure**: 98%+ profit margins with free tier hosting and GPT-4.1 Nano AI
- **Corporate Accounts**: Multi-recruiter teams with shared job postings and candidate collaboration

### Database Schema Design

#### Core Tables:
**Companies**
- id, company_name, billing_address, tax_id, industry, company_size, website
- stripe_customer_id, subscription_plan, billing_contact_email
- created_at, updated_at

**Recruiters**
- id, company_id, email, password_hash, first_name, last_name, phone
- role/title, permissions (admin/recruiter/viewer), active_status
- created_at, updated_at

**Jobs**
- id, company_id, primary_recruiter_id, title, description, requirements
- location, salary_min, salary_max, remote_ok, skills_required, experience_level
- status (active/closed), created_at, expires_at

**JobAssignments** (Many-to-Many)
- id, job_id, recruiter_id, role, assigned_at, assigned_by
- permissions (view/contact/edit)

**JobCandidates**
- id, job_id, candidate_id (user_id), assigned_recruiter_id
- match_score, recruiter_notes, status, contacted_at, contacted_by
- response_status, created_at

#### Supporting Tables:
**RecruiterSubscriptions**
- id, company_id, stripe_subscription_id, plan_type, billing_cycle
- amount_per_seat, seat_count, usage_searches, usage_contacts
- current_period_start, current_period_end, status

**CandidateViews**
- id, recruiter_id, candidate_id (user_id), viewed_at
- contact_attempted, notes

**SavedSearches**
- id, recruiter_id, search_name, criteria_json, location, radius
- skills, salary_range, email_alerts, last_run, created_at

**Messages**
- id, recruiter_id, candidate_id, message_text, sent_at, read_at, message_type

**DeviceTrials** (24-Hour Free Preview)
- device_fingerprint, first_access, trial_expired, ip_address, user_agent

### Progressive Disclosure UX Design
**Solo Recruiter Experience:**
- Auto-create company during signup, set as admin by default
- Simple "My Jobs" interface with no team management complexity
- Standard recruiting workflow without assignment features

**Team Features (Unlocked When Needed):**
- "Add Team Member" button appears after first recruiter invite
- Job sharing and assignment features become visible with 2+ team members
- Corporate billing and admin controls activate automatically

### Map Interface with Candidate Clustering
**Geographic Search:**
- Radius search using existing user lat/lon data with great circle distance calculation
- Candidate clustering by zip code with count badges to prevent dot overlap
- Click cluster opens candidate list popup/sidebar with profile details

**Cluster Implementation:**
```sql
SELECT zip_code, latitude, longitude, COUNT(*) as candidate_count,
       JSON_AGG(user_id) as candidate_ids
FROM users WHERE <search_criteria>
GROUP BY zip_code, latitude, longitude
```

### 24-Hour Free Preview System
...
- **0-24 hours**: Full candidate details visible
- **24+ hours**: Blur sensitive info → "John D.", "j***@***.com"
- **Map view**: Still shows dots with first names only
- **Conversion trigger**: "Subscribe to see full details" at 24-hour mark

**Anti-Circumvention:**
- Track IP + fingerprint combo, limit 3 trials per IP
- Server-side blur rendering (can't inspect element to unblur)

### Technical Implementation Notes
- **Existing Infrastructure**: Leverages current user lat/lon data, no additional geo tables needed
- **Team Collaboration**: Multiple recruiters can work same job posting without duplicate outreach
- **Corporate Billing**: Single invoice per company with per-seat pricing
- **Global Payments**: Stripe handles international tax, currency conversion, compliance
- **Lean Architecture**: Builds on existing free-tier hosting with minimal additional overhead

### Demo/Live Data Architecture (Session 8 Implementation)

**Table Structure:**
- **Live Tables**: `users`, `resumes`, `profiles` (production data)
- **Demo Tables**: `demo_users`, `demo_resumes`, `demo_profiles` (identical structure, populated with CSV demo data)

**Variable-Based Table Access:**
```python
# Core pattern: [prefix] + [table_name]
table_prefix = "demo_" if demo_mode else ""
query = f"SELECT * FROM {table_prefix}users WHERE..."
```

**Demo Mode Trigger:**
- **Landing Page Logic**: Paid subscription → Login to live system, Demo button → Demo system
- **Environment/Context**: Demo mode determined at recruiter entry point
- **Data Isolation**: Complete separation - demo recruiters only see demo candidates

**Benefits:**
- **Investor Demos**: Fully populated maps with realistic candidate data
- **Live System**: Real users and recruiters without demo pollution  
- **Seamless Switch**: Same code, different data source
- **Safe Testing**: Demo recruiters can't contact real candidates

## The Claude AI Development Experience

### Session 8 (2025-08-13) - "Dorothy Meets Groundhog Day"
**The Eternal OAuth Journey**
...
**The New Simple Flow:**
1. User lands on app (OAuth or manual signup)
2. Friendly welcome message, no auto-popups
3. User tries to generate resume
4. Helpful guidance appears: "Complete profile first" or "Upload documents first"
5. Natural discovery of Profile/Documents buttons
6. Success!

No more ruby slipper clicking, no more tornado-induced memory loss. Just a clean, intuitive experience that works every time. 
...
*"I'll get back to the spiral... after I fix this one more OAuth issue..."* 🌀💫

## 🎯 Mike Factor Configuration - IMPORTANT TUNING PARAMETER

**Location**: `G:\myresumechat\backend\app\routes\recruiter_app.py` lines 136 and 144

**Current Value**: `mike_factor = 2.0` (updated from 1.2 → 1.5 → 2.0)

**Purpose**: Real-world accuracy correction for commute polygon calculations. This multiplier adjusts the effective radius of commute areas based on user observations and testing.

**Usage**: 
- **Higher values (1.5+)**: Larger, more generous commute polygons
- **Lower values (0.8-1.2)**: Tighter, more conservative commute areas
- **Default was 1.2**, now **1.5** for 25% larger polygons

**Code Context**:
```python
# Main calculation (line 136)
mike_factor = 2.0
effective_radius = (minutes * 50.0 * mike_factor) / (adjusted_traffic_factor * 60.0)

# Fallback calculation (line 144) 
mike_factor = 2.0
effective_radius = (minutes * 50.0 * mike_factor) / (1.1 * 60.0)
```

**⚠️ CRITICAL**: This value may be adjusted multiple times based on real-world testing and user feedback. Always update BOTH instances when changing the value.
